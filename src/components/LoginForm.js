...
class LoginForm extends Component {
    ...
    onSubmit = async (event) => {
        ...
        this.drizzle.contracts.Identity.methods.signup(usernameHex).send({
            from: this.account
        })
    ...
    render() {
        ...
        if(this.drizzle.contracts.Identity
            ...
            const userKey = this.drizzle.contracts.Identity.methods.login.cacheCall()
            // Check that the data is cached
            if(this.props.contracts.Identity.login[userKey]
            ){
                // check to make sure has created an account
                if(this.props.contracts.Identity.login[userKey].value === undefined) {
                    user = ''
                }
                else {
                    user = this.drizzle.web3.utils.hexToUtf8(this.props.contracts.Identity.login[userKey].value)
                }
            }
        }
        return(
                ...
                ?
                    ...
                    <form onSubmit={this.onSubmit}>
                        <div className="field">
                            ...
                            <input type="text" className="input is-info" placeholder="John Doe" name="username"
