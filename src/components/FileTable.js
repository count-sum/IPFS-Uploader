...
class FileTable extends Component {
  ...
  constructor (props, context) {
    ...
    this.FileContractAddress = this.drizzle.contracts.FileContract.methods
    this.fileArray = []
    this.state = {
      lastIds: 0,
      table: [],
  ...
  componentDidMount () {
    ...
    this.drizzle.contracts.FileContract.methods.lastIds(this.state.fileOwnerAddress).call()
      .then((lastIds) => {
        this.setState({
          lastIds: lastIds
        })
    ...
    this.drizzle.contracts.FileContract.methods.lastIds(this.state.fileOwnerAddress).call()
      .then((lastIds) => {
        ...
        for (let i = 0; i < lastIds; i++) {
          this.drizzle.contracts.FileContract.methods.files(this.state.fileOwnerAddress, i).call()
            .then((fileItem) => {
              ...
              this.drizzle.contracts.FileContract.methods.getFileTags(this.state.fileOwnerAddress, i).call()
                .then((tags) => {
                  // console.log(tags)
    ...
    /** this.drizzle.contracts.FileContract.methods.files(this.fileOwnerAddress,0).call()
        .then((fileItem) => {
          console.log(fileItem)
          return fileItem
        })
  ...
  /**
    loadTableData() {
      const tableData = []
      const megaTest = this.drizzle.contracts.FileContract.methods.lastIds(this.fileOwnerAddress).call()
        .then((lastIds) => {
    ...
    */
  loadTableRow (index) {
    this.drizzle.contracts.FileContract.methods.files(this.state.fileOwnerAddress, 0).call()
      .then((fileItem) => {
        return fileItem
      })
  }
          ...
          <table className='table'>
            <thead>
              <tr>
                ...
                <th><i className='fas fa-user' /> Eth Address</th>
                <th><abbr title='IPFS Unique Identifier'> <i className='fas fa-hashtag' /> Ipfs Hash </abbr> </th>
                <th><abbr title='Unix Timestamp'> <i className='fas fa-clock' /> Created on </abbr></th>
...
const mapStateToProps = state => {
  return {
    ...
    FileContract: state.contracts.FileContract,
    web3: state.web3
  }
}
